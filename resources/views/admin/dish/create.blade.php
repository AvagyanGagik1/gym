            ...
            <div class="form-group row pb-3">
                ...
                <div class="col-sm-10">
                    <select class="custom-select" id="food_category_id" name="food_category_id">
                        <option disabled @if(!old('food_category_id')) selected @endif>--Выберите Категорию для питания--</option>
                        @foreach($foodCategory as $category)
                            <option value="{{$category->id}}" @if(old('food_category_id')=== $category->id) selected @endif>{{$category->name_ru}}</option>
                        @endforeach
                    </select>
