<footer class="d-flex flex-column">
    <div>
        <img class="absoluteTop" src="images/top.png" alt="">
    </div>
    <div class="col-10 footerNav">
        <div class="row  d-flex justify-content-between align-items-center">
            <a href="#" class="footer-main-link">
                <img src="images/logo.png" alt="">
            </a>
            <div>
                <ul>
                    <a href="">
                        <li>Программы тренировок</li>
                    </a>
                    <a href="">
                        <li>Тренеры</li>
                    </a>
                    <a href="">
                        <li>Результаты</li>
                    </a>
                    <a href="">
                        <li>Программы подписки</li>
                    </a>
                    <a href="">
                        <li>Блог</li>
                    </a>
                </ul>
            </div>
            <div class="footerClient">
                <a href="">
                    <span>ЛИЧНЫЙ КАБИНЕТ</span>
                </a>
            </div>
        </div>
    </div>
    <div class="col-10 line"></div>
    <div class="col-10">
        <div class="footerPromo row d-flex justify-content-between align-items-center">
            <div class="d-flex align-items-center">
                <p>Наш партнер:</p>
                <img src="images/club.png" alt="">
            </div>
            <div class="d-flex align-items-center">
                <h6>Copyright © 2018-2021 Online Gym project. All Rights Reserved.
                </h6>
                <img src="images/facebook.png" alt="">
                <img src="images/twitter.png" alt="">
                <img src="images/instagram.png" alt="">
            </div>
            <div class="d-flex align-items-center">
                <h6>Разработано:</h6>
                <img src="images/PROMODEX.png" alt="">
            </div>
        </div>
    </div>
</footer>
